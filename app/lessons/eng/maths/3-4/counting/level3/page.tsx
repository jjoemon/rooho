...
export default function CountingLevel3() {
  ...
  useEffect(() => {
    ...
    const utter = new SpeechSynthesisUtterance("Let's count together! One to seven!");
    utter.rate = 0.9;
  ...
  useEffect(() => {
    ...
    const explain = () => {
      if (step >= numberData.length) {
        const utter = new SpeechSynthesisUtterance("Now you learned one to seven! Let's count again!");
        utter.rate = 0.95;
        window.speechSynthesis.cancel();
        window.speechSynthesis.speak(utter);
  ...
  useEffect(() => {
    ...
    const repeat = () => {
      if (step >= numberData.length) {
        setCurrentNumber(numberData.length); // trigger all numbers display
        return;
      }
      const num = numberData[step];
      setCurrentNumber(num.number);
  ...
  return (
    <LessonTemplate
      title="Counting Level 3"
      description="Learn to count from 1 to 7 with flowers!"
      onPlayAgain={handlePlayAgain}
      continuePath="/lessons/eng/maths/3-4/counting/level4"
    >
